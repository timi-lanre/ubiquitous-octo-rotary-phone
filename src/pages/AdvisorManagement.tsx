...
import { SelectedFilters } from '@/components/FilterPanel/FilterPanelTypes';
import { Plus, Pencil, Trash2, Loader2, ChevronUp, ChevronDown } from 'lucide-react';
import { buildQuery } from '@/hooks/useAdvisors/queryBuilders';
...
export default function AdvisorManagement() {
  ...
  const { data: totalCount } = useQuery({
    queryKey: ['advisors-management-count', debouncedSearchQuery, selectedFilters, favoriteAdvisorIds, reportAdvisorIds],
    queryFn: async () => {
      console.log('🔍 Fetching total count with params:', {
      ...
      if (!query) {
        console.log('❌ No query built, returning 0');
        return 0;
      }
      
      // Use the correct Supabase syntax for count only
      ...
      <main className="px-2 lg:px-4 pb-12 pt-6">
        <div className="w-full max-w-none">
          <div className="mb-6">
            ...
            <Card>
              <CardHeader>
                <CardTitle>Advisors ({(totalCount || 0).toLocaleString()})</CardTitle>
                <CardDescription>
