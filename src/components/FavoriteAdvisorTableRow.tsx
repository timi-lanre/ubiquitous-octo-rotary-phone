...
import { Mail, Linkedin, Globe, Info, Heart, Trash2, Flag } from 'lucide-react';
...
export function FavoriteAdvisorTableRow({ 
  advisor, 
  favoriteListId, 
  favoriteItemId,
  index 
}: FavoriteAdvisorTableRowProps) {
  ...
  const removeFromFavoritesMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description: `${advisor.first_name} ${advisor.last_name} has been removed from this list.`,
      });
    },
    onError: (error) => {
      toast({
        title: "Error",
        description: "Failed to remove advisor from favorites.",
        variant: "destructive",
      });
    },
  });
  ...
  const handleRemoveFromFavorites = () => {
    if (window.confirm(`Remove ${advisor.first_name} ${advisor.last_name} from this favorites list?`)) {
